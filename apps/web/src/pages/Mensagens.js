...
export function Mensagens() {
    ...
    return (_jsx(PageTransition, { children: _jsxs("div", { className: "h-[calc(100vh-2rem)]", children: [_jsx(PageHeader, { title: "Mensagens", subtitle: "Comunicacao com motoristas" }), _jsxs("div", { className: "flex gap-6 h-[calc(100%-5rem)]", children: [_jsxs("div", { className: "w-80 border border-border/30 rounded-xl flex flex-col overflow-hidden", children: [_jsx("div", { className: "p-4 border-b border-border/30", children: _jsx("h2", { className: "text-sm font-semibold text-text", children: "Conversas" }) }), _jsx("div", { className: "flex-1 overflow-y-auto", children: loading ? (_jsx("div", { className: "p-4 text-center text-text-muted text-sm", children: "Carregando..." })) : conversas.length === 0 && motoristasWithoutConvo.length === 0 ? (_jsx("div", { className: "p-4 text-center text-text-muted text-sm", children: "Nenhuma conversa ou motorista disponivel" })) : (_jsxs(_Fragment, { children: [conversas.map((conversa) => (_jsxs("button", { onClick: () => setSelectedConversation(conversa), className: clsx('w-full p-4 flex items-start gap-3 hover:bg-surface2/50 transition-colors text-left border-b border-border/30', selectedConversation?.participante_id === conversa.participante_id &&
                                                ...
                                                return (_jsx("div", { className: clsx('flex', isFromMe ? 'justify-end' : 'justify-start'), children: _jsxs("div", { className: clsx('max-w-[70%] px-4 py-2 rounded-2xl', isFromMe
                                                            ...
                                                            : 'bg-surface2 text-text rounded-bl-sm'), children: [_jsx("p", { className: "text-sm whitespace-pre-wrap break-words", children: msg.conteudo }), _jsx("p", { className: clsx('text-xs mt-1', isFromMe ? 'text-text-muted' : 'text-text-muted'), children: formatTime(msg.criado_em) })] }) }, msg.id));
                                            })), _jsx("div", { ref: messagesEndRef })] }), _jsx("div", { className: "p-4 border-t border-border/30", children: _jsxs("div", { className: "flex gap-2", children: [_jsx("input", { type: "text", value: newMessage, onChange: (e) => setNewMessage(e.target.value), onKeyDown: handleKeyDown, placeholder: "Digite sua mensagem...", className: "flex-1 bg-surface2 border border-border/30 rounded-xl px-4 py-3 text-text text-sm focus:outline-none focus:border-accent placeholder-gray-500" }), _jsx("button", { onClick: sendMessage, disabled: !newMessage.trim() || sending, className: clsx('px-4 py-3 rounded-xl transition-colors', newMessage.trim() && !sending
                                                        ? 'bg-accent hover:bg-accent/90 text-text'
                                                        : 'bg-surface2 text-text-muted cursor-not-allowed'), children: _jsx(Send, { size: 18 }) })] }) })] })) : (_jsx(EmptyState, { icon: MessageCircle, message: "Selecione uma conversa para comecar" })) })] })] }) }));
}
